...
y qué concluimos**. Es el corazón del entregable, porque el valor no es solo "que
funcione" sino *haber investigado*.

Cada entrada se mide contra anotación humana: el lote de 16 notas doble-anotadas
(acuerdo entre anotadores **F1 0.71**) y, desde el Exp 5, un held-out de 75
notas. Cada experimento es reproducible con su propio script
(`python -m experiments.<exp>`; sin API keys corren offline desde los caches).

> **Cómo leer las métricas:** P = precisión (de lo que detecté, cuánto era fuente
> real), R = recall (de las fuentes reales, cuántas detecté), F1 = media armónica.
...
| 4 | 2026-08-07 | Citas implícitas (exploratorio, n=7) | - | - | - | LLM detecta 5/7 débiles vs clásico 2/7; el flag `explicita` casi no se usa |
...
**Qué funcionó.** El LLM **duplica** el F1 del clásico y tiene recall alto (0.78):
detecta instituciones y atribuciones parafraseadas que el clásico (solo comillas)
...
- **El modelo importa más que el prompt… salvo que se acierte el prompt.** Con
  el mismo prompt, Sonnet supera a Gemini por +0.26 (`v0`), +0.30 (`v1`) y +0.27
  (`v2`). Pero con `v3_justifica` **la brecha cae a +0.07** (0.74 vs
...
- **El mejor prompt DEPENDE del modelo, y no es un detalle: se invierte.**
  ...
  pedir que cite la evidencia o descarte el candidato corrige en el modelo débil
  justo su defecto (sobre-detectar), mientras que al fuerte, que ya parte de
  P 0.91, solo lo vuelve conservador de más y le cuesta recall. **Corolario
...
- **La dirección "subir precisión" se confirma en ambos**, pero con palancas
  ...
  la auto-verificación la sube muchísimo solo en el modelo más pequeño.
- **Sonnet supera el "techo humano" (0.71).** Conviene leerlo con cuidado: ese techo es el
  **acuerdo entre los dos anotadores** (lch vs xig), no un máximo teórico. Que el
  ...
  del desacuerdo humano con 16 notas. **Y el 0.86 no se sostiene: el [Exp 5](#exp-5--validación-held-out-el-086-generaliza)
...
- **`v3_justifica` se caía por nuestra propia config, no (solo) por el modelo:**
  ...
  La moraleja: **un error de parseo puede disfrazarse de "el modelo es malo"**: antes de concluir, mirar la respuesta cruda.
- **La auto-verificación no ayudó *en Sonnet*:** v3 da 0.81: baja recall (0.75)
  sin ganar precisión (0.88 < 0.94). Conviene no generalizar: en Gemini la misma
  variante es la ganadora con claridad. Es la evidencia más clara del proyecto de
...
- **La cuota diaria del free tier de Gemini se agotó** a mitad de corrida (429
  ...
  decisión de no publicarlas se confirmó:** en 8/16 daba F1 **0.87** y
...
rate-limit. Cada llamada fallida gatilla reintentos que **consumen la cuota
...
## Exp 2: salida rica v1 (el output requerido)
...
**Qué funcionó.** El **output** ya tiene la forma pedida, con posiciones correctas
...
lista de fuentes Sonnet superaba a Gemini por +0.26 a +0.30; acá, con el mismo prompt v1, la
...
fuente*, y ahí el modelo más pequeño alcanza. La ventaja del modelo caro se concentra
justo en la parte de juicio.

**Qué funcionó.** Los dos LLM superan al clásico **en los tres componentes**, y
...
marca "el gobernador Martín Llaryora" y el modelo copia "Llaryora" (o al revés): con IoU ≥ 0.5 eso cuenta como error aunque la fuente esté bien identificada. El
...
**Idea (propuesta de la cátedra).** En vez de resolver todo en una sola llamada,
...
**Por qué esta config vale como respuesta a la propuesta de la cátedra.** La idea era
...
parecía fallar por completo. Pero mirando el cache: **10 de las 16 notas devolvían 0
...
**Fecha:** 2026-08-07 · **Estado:** ✅ medido · **n bajo: leer con cautela**

**Qué es.** La consigna pide tener en cuenta las **citas implícitas** (atribución
...
**Qué funcionó.** La dirección esperada se confirma: el clásico (que necesita
comillas o verbo de habla por diseño) pierde las implícitas (0.29), mientras el
LLM las detecta casi al mismo nivel que las explícitas (0.71 vs 0.75). Es el
...
Sonnet: (a) los prompts se **eligieron** mirando esas mismas 16 notas (sobreajuste de selección), y (b) las 16 doble-anotadas son un subconjunto
...
- **El 0.86 no se sostiene: en notas no vistas es 0.67.** Pero la brecha se reparte
  entre tres componentes de tamaño muy distinto:
  1. **Sobreajuste de selección: acotado.** La ventaja del few-shot sobre v0 pasa
...
0.00); `gemini·v0` resuelve las "fáciles" pero colapsa en el tramo difícil de
...
mejora al modelo más pequeño de forma pareja: le arregla justo las notas difíciles,
...
**Qué funcionó / Qué no.** <análisis de aciertos y fallas, con ejemplos>
